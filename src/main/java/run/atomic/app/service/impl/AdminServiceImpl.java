...
import run.atomic.app.model.dto.EnvironmentDTO;
import run.atomic.app.model.dto.StatisticDTO;
...
import run.atomic.app.model.params.LoginParam;
import run.atomic.app.model.params.ResetPasswordParam;
import run.atomic.app.model.properties.EmailProperties;
import run.atomic.app.model.support.HaloConst;
import run.atomic.app.service.*;
import run.atomic.app.utils.FileUtils;
import run.atomic.app.utils.HaloUtils;
...
import run.atomic.app.security.authentication.Authentication;
import run.atomic.app.security.context.SecurityContextHolder;
import run.atomic.app.security.token.AuthToken;
import run.atomic.app.security.util.SecurityUtils;
import run.atomic.app.service.*;
...
public class AdminServiceImpl implements AdminService {
    ...
    public void updateAdminAssets() {
        ...
        if (responseEntity == null ||
            ...
            log.debug("Failed to request remote url: [{}]", HaloConst.HALO_ADMIN_RELEASES_LATEST);
            throw new ServiceException("系统无法访问到 Github 的 API").setErrorData(HaloConst.HALO_ADMIN_RELEASES_LATEST);
        }
        ...
        if (assetsObject instanceof List) {
            try {
                ...
                String adminTargetName = haloProperties.getWorkDir() + HaloConst.HALO_ADMIN_RELATIVE_PATH;

                Path adminPath = Paths.get(adminTargetName);
                Path adminBackupPath = Paths.get(haloProperties.getWorkDir(), HaloConst.HALO_ADMIN_RELATIVE_BACKUP_PATH);
    ...
    private Predicate<Object> assetPredicate() {
        return asset -> {
            ...
            return name.toString().matches(HaloConst.HALO_ADMIN_VERSION_REGEX) && "application/zip".equalsIgnoreCase(contentType);
        };
    }
    ...
    private void backupAndClearAdminAssetsIfPresent(@NonNull Path sourcePath, @NonNull Path backupPath) throws IOException {
        ...
        if (!FileUtils.isEmpty(sourcePath)) {
            // Clone this assets
            Path adminPathBackup = Paths.get(haloProperties.getWorkDir(), HaloConst.HALO_ADMIN_RELATIVE_BACKUP_PATH);
